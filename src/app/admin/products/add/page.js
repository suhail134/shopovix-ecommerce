...
const removeExistingImage = (index) => {
  ...
  setDeletedImages(prev => [...prev, imgToDelete.public_id]); // add public_id to deletedImages
...
const removeNewImage = (index) => {
  ...
  const handleSubmit = async (e) => {
    ...
    if (id && images.length === 0 && (images.length + existingImages.length) === 0) {
          ...
          <div className="bg-white p-6 rounded-xl shadow-md">
            ...
            <div className="flex flex-wrap gap-4">
              ...
              {existingImages.map((img, idx) => (
                <div key={`existing-${idx}`} className="relative w-28 h-28 border rounded overflow-hidden">
                  <img
                    src={img.url} // ya img directly agar sirf URL ho
